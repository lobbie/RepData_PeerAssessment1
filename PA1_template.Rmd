...
```
<br>
The plot above is the **daily average steps by 5-minutes intervals** across all days.  The time interval **`r paste(varMaxAvgSteps$interval)`** minutes have the highest average number of steps at **`r paste(varMaxAvgSteps$varAvgSteps)`** steps.
...
```
The total number of rows with missing values in variable 'steps' is **`r paste(varMissing)`**.
<br>
<br>
The strategy to impute these  **`r paste(varMissing)`** observations with missing values in variable 'steps' is to replace them with the median of total number of steps taken for that time interval in a new dataframe **dfActivityNew**.
```{r}
